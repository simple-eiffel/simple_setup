...
feature {NONE} -- Script Generation
	...
	generate_header (a_version: STRING): STRING
		...
		do
			...
			l_template.set_variable ("app_id", app_guid)
			l_template.set_variable ("github_org", manifest.github_org)
	...
	generate_files: STRING
		...
		do
			...
			Result.append ("; Source files from install directory, excluding build artifacts%N")

			across manifest.libraries as l_cursor loop
				Result.append ("Source: %"" + install_directory + "\" + l_cursor.name + "\*%"; ")
				Result.append ("DestDir: %"{app}\" + l_cursor.name + "%"; ")
				Result.append ("Flags: ignoreversion recursesubdirs createallsubdirs; ")
				Result.append ("Excludes: %"EIFGENs\*,.git\*,*.obj,*.exe,*.log,nul,*.pdb,*.lib,*.c,*.h%"%N")
			end
...
AppPublisherURL=https://github.com/{{github_org}}
AppSupportURL=https://github.com/{{github_org}}
AppUpdatesURL=https://github.com/{{github_org}}
...
PrivilegesRequired=lowest
	...
	app_guid: STRING = "{{F8A3B2C1-4D5E-6F7A-8B9C-0D1E2F3A4B5C}"
			-- Application GUID for Inno Setup (double {{ to escape literal brace)
